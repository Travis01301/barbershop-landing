...
export const CustomerHistory: React.FC<CustomerHistoryProps> = ({ 
  ...
  const getStatusColor = (status: string) => {
    switch (status.toLowerCase()) {
      ...
      default:
        return 'bg-gray-100 dark:bg-slate-800 text-gray-800';
    }
  };
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-lg shadow-md overflow-hidden">
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            {appointments.map((appointment) => (
              <tr key={appointment.id} className="hover:bg-gray-50 dark:bg-slate-900 transition-colors">
      ...
      {/* Statistics Footer */}
      <div className="px-6 py-4 bg-gray-50 dark:bg-slate-900 border-t border-gray-200">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            <p className="text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase">Total Visits</p>
            <p className="text-2xl font-bold text-gray-900">{appointments.length}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase">Completed</p>
            <p className="text-2xl font-bold text-green-600">
              {appointments.filter(a => a.status.toLowerCase() === 'completed').length}
            </p>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase">Cancelled</p>
            <p className="text-2xl font-bold text-red-600">
              {appointments.filter(a => a.status.toLowerCase() === 'cancelled').length}
            </p>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase">Total Spent</p>
            <p className="text-2xl font-bold text-green-600">
